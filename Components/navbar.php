<nav class="flex w-full backdrop-blur-md text-black p-3 z-40 shadow-lg font-[Inter] fixed justify-around">
    <ul class="flex justify-start gap-4 list-none m-0 p-0">
        <li class="inline-block px-4 py-2">
            <!-- <img src="/KosPelitaHarapan/assets/logo.png" class="size-20"> -->
        </li>
        <li class="inline-block px-4 py-2"> <a href="about.php">Booking</a> </li>
        <li class="inline-block px-4 py-2"> <a href="contact.php">Login</a> </li>
    </ul>
    <a type="button" class="text-gray-900 hover:text-white border border-gray-800 hover:bg-gray-900 focus:ring-4 focus:outline-none focus:ring-gray-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center me-2 mb-2 dark:border-gray-600 dark:text-gray-400 dark:hover:text-white dark:hover:bg-gray-600 dark:focus:ring-gray-800" href="login">Sign Up</a>

    <link href="https://cdn.jsdelivr.net/npm/daisyui@5/themes.css" rel="stylesheet" type="text/css" />
</nav>  